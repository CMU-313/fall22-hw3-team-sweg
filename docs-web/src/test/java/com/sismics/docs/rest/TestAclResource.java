...
public class TestAclResource extends BaseJerseyTest {
        ...
        public void testAclResource() {
                ...
                // Get messages for acl2
                json = target().path("/messages").request().cookie(TokenBasedSecurityFilter.COOKIE_NAME, acl2Token)
                                .get(JsonObject.class);
                JsonArray messages = json.getJsonArray("messages");
                Assert.assertEquals(1, messages.size());
                JsonObject message = messages.getJsonObject(0);
                Assert.assertEquals(MessageType.DOCUMENT_ASSIGNED.name(), message.getString("type"));
